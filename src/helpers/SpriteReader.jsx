...
export default function SpriteReader(props) {
  ...
  useEffect(() => {
    ...
    if (props.props.snippet.id === 500) {
      ...
      const frameData = config.frames[`${props.props.snippet.src} ${displayFrame}.aseprite`];
      console.log(1)
   ...
   else if (props.props.snippet.id) try{ //draw tiles
  ...
  useEffect(()=>{
    if(props.props.snippet.state==="idle"){
      const interval = setInterval(setCurrentFrame(currentFrame+1),100)
